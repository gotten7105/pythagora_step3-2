...
export default function Myparty3() {
    ...
    const handleGuestChange = () => {
        const url = `/cheers/guest?guest_email=${guest_email}`;
        window.open(url, '_blank');
    };

    const handleRecommendChange = () => {
        const url = `/cheers/recommend?select=${binaryString}${binaryString2}`;
        window.open(url, '_blank');
    };
    ...
    const handleRestaurantChange = async() => {
        ...
        const response = await fetch('http://127.0.0.1:5000/restaurant', {
            ...
            if (response.ok) {
                ...
                router.push(`/cheers/update3?event_id=${event_id}`);
                // alert("Check")
            }
    };
    ...
    return (
        (<div key="1" className="max-w-4xl mx-auto my-8 p-8 bg-white rounded-lg">
            ...
            <Button onClick={handleRecommendChange}>cheersレコメンド機能</Button>
            </div>
            <hr style={{ margin: '0 auto',  borderWidth: '2px' }}/>
            <div className="grid md:grid-cols-3 gap-4 my-5">
                ...
                <Button className="self-end md:col-span-1">店舗IDを検索</Button>
            </div>
